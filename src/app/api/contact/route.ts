...
export async function POST(request: Request) {
  try {
    ...
    // ni en la respuesta automática, que sale a la dirección que ponga cualquiera.
    const h = (s: unknown, max: number) =>
      String(s ?? "").slice(0, max).replace(/[&<>"']/g, (c) => `&#${c.charCodeAt(0)};`);
    ...
    const aviso = await resend().emails.send({
      ...
      to: OWNER_EMAIL,
      replyTo: email,
      subject: `🔔 Nuevo lead: ${String(name).slice(0, 100)} — ${String(service || "Consulta general").slice(0, 100)}`,
      html: `
        <!DOCTYPE html>
        <html>
            ...
            <div style="padding: 32px;">
              <table style="width: 100%; border-collapse: collapse;">
                <tr>
                  ...
                  <td style="padding: 10px 0; border-bottom: 1px solid #f0f0f5;">
                    <span style="color: #111; font-weight: 600;">${h(name, 100)}</span>
                  </td>
                </tr>
                ${company ? `
                <tr>
                  ...
                  <td style="padding: 10px 0; border-bottom: 1px solid #f0f0f5;">
                    <span style="color: #111;">${h(company, 150)}</span>
                  </td>
                </tr>` : ""}
                <tr>
                  <td style="padding: 10px 0; border-bottom: 1px solid #f0f0f5;">
                  ...
                  <td style="padding: 10px 0; border-bottom: 1px solid #f0f0f5;">
                    <a href="mailto:${h(email, 254)}" style="color: #b8860b; font-weight: 600; text-decoration: none;">${h(email, 254)}</a>
                  </td>
                </tr>
                ${service ? `
                <tr>
                  ...
                  <td style="padding: 10px 0; border-bottom: 1px solid #f0f0f5;">
                    <span style="background: #b8860b1a; color: #b8860b; font-size: 12px; font-weight: 700; padding: 3px 10px; border-radius: 20px;">${h(service, 100)}</span>
                  </td>
                </tr>` : ""}
              </table>
              ...
              <div style="margin-top: 24px;">
                ...
                <div style="background: #f8f8ff; border-left: 3px solid #b8860b; padding: 16px; border-radius: 0 8px 8px 0; color: #333; line-height: 1.6; font-size: 15px;">
                  ${h(message, 5000).replace(/\n/g, "<br>")}
                </div>
              </div>

              <div style="margin-top: 28px; display: flex; gap: 12px;">
                <a href="mailto:${h(email, 254)}?subject=Re: Tu consulta a Josue Solorzano"
        ...
        <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; background: #f4f4f8; margin: 0; padding: 24px;">
          <div style="max-width: 560px; margin: 0 auto; background: white; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 24px rgba(0,0,0,0.08);">
            <div style="background: linear-gradient(135deg, #b8860b, #7a5500); padding: 32px; color: white; text-align: center;">
              ...
              <h1 style="margin: 0; font-size: 22px; font-weight: 700;">¡Recibí tu mensaje, ${h(String(name).split(" ")[0], 40)}!</h1>
            </div>
    ...
    // El aviso ya llegó: si falla sólo el acuse al visitante, no se le dice que falló.
    if (acuse.error) console.error("Contact API acuse:", acuse.error);
