...
const categories = ["Todos", "Diseño Web", "SEO", "Inteligencia Artificial", "Marketing Digital", "Leads", "Autoridad Digital", "Negocios Globales"];
...
export default function BlogPage() {
  ...
  return (
    <>
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(webPageSchema({ title: "Blog de Josue Solorzano", description: "Artículos sobre liderazgo, emprendimiento y mentalidad.", url: `${siteConfig.url}/blog`, breadcrumbs })) }} />
      ...
      <section className="pt-32 pb-12 relative">
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <span className="text-[#f0c040] text-sm font-semibold uppercase tracking-widest">Estrategias & Perspectivas</span>
          <h1 className="text-5xl lg:text-6xl font-bold text-white mt-3 mb-6">
            El Blog de <span className="gradient-text">Josue</span>
          </h1>
          <p className="text-[#8888aa] text-xl leading-relaxed max-w-2xl mx-auto">
            Guías prácticas sobre sitios web, SEO, inteligencia artificial y marketing digital. Sin teoría vacía — solo lo que funciona para atraer clientes reales.
          </p>
        </div>
      </section>
            ...
            <button
              ...
              className={`text-sm px-4 py-1.5 rounded-full transition-all ${
                cat === "Todos"
                  ...
                  : "border border-[#1e1e2e] text-[#8888aa] hover:text-white hover:border-[#b8860b33]"
              }`}
            >
              {cat}
            </button>
      ...
      <section className="pb-12">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-3 gap-6 mb-16">
            {featured.map((post) => (
              <Link key={post.slug} href={`/blog/${post.slug}`} className="group bg-[#111118] border border-[#1e1e2e] rounded-2xl overflow-hidden hover:border-[#b8860b33] hover:-translate-y-1 transition-all">
                ...
                <div className="p-5">
                  <span className="text-[#f0c040] text-xs font-semibold">{post.category}</span>
                  <h3 className="text-white font-bold text-base mt-2 mb-3 line-clamp-2 group-hover:text-[#f0c040] transition-colors">
                    {post.title}
                  </h3>
                  <p className="text-[#8888aa] text-sm line-clamp-2 mb-4">{post.excerpt}</p>
          ...
          <div className="space-y-4">
            {rest.map((post) => (
              <Link key={post.slug} href={`/blog/${post.slug}`} className="group flex gap-5 p-5 bg-[#111118] border border-[#1e1e2e] rounded-xl hover:border-[#b8860b33] transition-all">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <h3 className="text-white font-semibold text-sm group-hover:text-[#f0c040] transition-colors line-clamp-1">{post.title}</h3>
                  <p className="text-[#8888aa] text-xs mt-1 line-clamp-1">{post.excerpt}</p>
                </div>
