...
export default async function BlogPostPage({ params }: Props) {
  ...
  return (
    <>
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(articleSchema({ ...post, image: post.image })) }} />
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(webPageSchema({ title: post.title, description: post.excerpt, url: `${siteConfig.url}/blog/${post.slug}`, breadcrumbs })) }} />
      ...
      <section className="pt-32 pb-12 relative">
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-6">
            <span className="text-[#a78bfa] text-xs font-semibold uppercase tracking-widest bg-[#7c3aed1a] border border-[#7c3aed33] px-3 py-1 rounded-full">
              {post.category}
            </span>
          </div>

          <h1 className="text-4xl lg:text-5xl font-bold text-white leading-tight mb-6">{post.title}</h1>
          <p className="text-[#8888aa] text-xl leading-relaxed mb-8">{post.excerpt}</p>
          ...
          <div className="flex items-center gap-6 text-sm text-[#8888aa] pb-8 border-b border-[#1e1e2e]">
            ...
            <span className="flex items-center gap-1.5"><Clock size={13} /> {post.readingTime} min de lectura</span>
          </div>
        </div>
      </section>
      ...
      <section className="py-12">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          <article className="prose prose-invert prose-lg max-w-none prose-headings:text-white prose-headings:font-bold prose-p:text-[#ccccdd] prose-p:leading-relaxed prose-strong:text-white prose-a:text-[#a78bfa] prose-code:text-[#a78bfa] prose-blockquote:border-[#7c3aed] prose-blockquote:text-[#8888aa]">
            {post.content.split("\n").map((line, i) => {
          ...
          <div className="mt-10 pt-8 border-t border-[#1e1e2e]">
            <p className="text-[#8888aa] text-sm mb-4">¿Te fue útil? Compártelo:</p>
          ...
          <div className="flex flex-wrap gap-2 mt-8 pt-8 border-t border-[#1e1e2e]">
            {post.tags.map((tag) => (
              <span key={tag} className="text-xs text-[#8888aa] border border-[#1e1e2e] px-3 py-1 rounded-full hover:border-[#7c3aed33] hover:text-[#a78bfa] transition-all cursor-pointer">
                #{tag}
              </span>
            ))}
          </div>
          ...
          <div className="mt-10 p-6 bg-[#111118] border border-[#1e1e2e] rounded-2xl flex gap-5">
            ...
            <div>
              ...
              <p className="text-[#8888aa] text-sm mt-1 leading-relaxed">
                Especialista en servicios digitales premium desde Costa Rica. Ayuda a negocios en USA, Europa y Latinoamérica a dominar internet con sitios web, SEO y optimización para IAs.
              </p>
              <Link href="/sobre-mi" className="text-[#a78bfa] text-sm font-medium hover:text-white transition-colors mt-2 inline-block">
                Conoce más →
              </Link>
            </div>
          </div>
        ...
        <section className="py-16 bg-[#0d0d14]">
          <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid md:grid-cols-3 gap-6">
              {relatedPosts.map((p) => (
                <Link key={p.slug} href={`/blog/${p.slug}`} className="group bg-[#111118] border border-[#1e1e2e] rounded-xl p-5 hover:border-[#7c3aed33] transition-all">
                  <span className="text-[#a78bfa] text-xs font-semibold">{p.category}</span>
                  <h3 className="text-white font-semibold text-sm mt-2 mb-2 line-clamp-2 group-hover:text-[#a78bfa] transition-colors">{p.title}</h3>
                  <p className="text-[#8888aa] text-xs line-clamp-2">{p.excerpt}</p>
                  <div className="flex items-center gap-1 text-[#a78bfa] text-xs mt-3 font-medium">
      ...
      <div className="py-8 max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        <Link href="/blog" className="inline-flex items-center gap-2 text-[#8888aa] hover:text-white transition-colors text-sm">
          <ArrowLeft size={14} />
          Volver al Blog
        </Link>
      </div>
